...
import pandas as pd
import numpy as np
from supplemental_functions import haversine, streamData, creating_fig, sliding_window, wavelength
import os
...
plotType = 'cc'
twRange = np.arange(0.15, 0.71, 0.01)
...
for num in range(event, len(picks)):  # event
    ...
    if (save_figs or plot_figs):
        ...
        im = ax2[0].imshow(RecMat, origin='lower', extent=extent, aspect="auto", alpha=0.7, cmap="plasma")
        ...
        ax2[0].set_ylabel(r'$\delta$t [s]')
        plt.setp(ax2[0].get_xticklabels(), visible=False)
        # ax2[0].scatter(dts[dtMax1], angstmp[angMax1], marker='*', s=40, zorder=10, c="tab:blue", ec='k', lw=0.5,
        #                label='95%rl max')

        im = ax2[1].imshow(CCMat, origin='lower', extent=extent, aspect="auto", alpha=0.7, cmap="plasma")
        ...
        # ax2[1].scatter(dts[dtMaxI], angs[angMaxI], marker='*', s=40, zorder=10, c="tab:red", ec='k',
        #                lw=0.5, label=f'{int(cc_percent * 100)}%cc max')
        ...
        ax2[2].set_ylabel(r'$\delta$t [s]')
        ax2[2].set_xlabel(r'$\phi^{\circ}$')

        handles, labels = [(a + b + c) for a, b, c in zip(*[ax2[i].get_legend_handles_labels() for i in range(3)])]
        fig.legend(handles, labels, loc=[0.86, 0.76])
        ...
        if save_figs:
            save_pathtmp = os.path.join(save_path, row.station)
            if not os.path.exists(save_pathtmp):
                os.makedirs(save_pathtmp)
            fig.savefig(os.path.join(save_pathtmp, f'{row.evid}.png'), bbox_inches='tight', pad_inches=0.1, dpi=400)
...
if save_df:
    df.to_csv('SWS_CC.csv', index=False)
